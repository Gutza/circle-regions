...
graph.addCircle(new Circle(graph, {x: 0, y: 5}, 5));

describe("Tangency groups for five circles AA-BB + C", () => {
    dumpGroups("Five", graph, "!!Look out for this tag (it shows multiple times)!!");
    ...
    it("Five tangent circles AA-BB + C should contain two tangent groups", () => {
        assert.equal(tanGroupCount, 2);
    });  
});
